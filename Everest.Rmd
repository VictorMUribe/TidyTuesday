...
```{r}
set.seed(5302)
sig_mod <- logistic_reg(penalty = 0.1, mixture = 1) %>% 
  set_engine("glmnet") %>% 
  fit(success ~ year + age + sex + hired + oxygen_used +
        died + injured, data = train_data)

```

Note: season was pulled due to the following error

\[\text{Warning: glm.fit: fitted probabilities numerically 0 or 1 occurred}\]
...
factor(ifelse(p == TRUE, TRUE, FALSE)) %>% 
  caret::confusionMatrix( test_data[["success"]])
```
...
|---      |---       |---          |---   
|  Lasso  |  0.9009  |    0.8097   |  1.0000     
|  AIC    |  0.9139  |    0.8515   |  0.9817
